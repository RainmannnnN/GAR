...
class HEATER:
    def __init__(self, sess, args, emb_dim, content_dim):
        ...
        self.expert_layers = [3 * self.emb_dim]
        ...
        self.d_act = 'tanh'
        self.drop_emb_rate = args.drop_emb_rate
        ...
        Vcontent_last = tf.reshape(Vcontent_last, [-1, 3, self.emb_dim])
        expand_indicator = tf.tile(tf.expand_dims(self.dropout_item_indicator, axis=1), [1, 3, 1])
        self.V_embedding = (self.Vin * expand_indicator + Vcontent_last * (1 - expand_indicator))
        ...
        self.reg_loss += tf.add_n(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES, scope='D'))
        self.loss = self.target_loss + self.reg_loss + self.alpha * self.diff_loss
    ...
    def train_bce(self, v_content, v_emb, u_emb, target):
        ...
        drop_indicator = np.random.uniform(size=[len(target), 1])
        drop_indicator = (drop_indicator > self.drop_emb_rate).astype(np.float32)
        _train_dict = {
