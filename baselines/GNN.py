...
class GNN(object):
    def __init__(self, sess, args, emb_dim, content_dim):
        ...
        uemb = tf.tile(self.opp_emb, [2, 1, 1])
        iemb = tf.concat([self.real_emb, self.neg_emb], axis=0)
        D_out = self.build_discriminator(uemb, iemb,
                                         self.d_layer, self.d_act, self.d_drop, self.d_training, False, False)

        self.D_out = tf.transpose(tf.reshape(D_out, [2, -1]))
        ...
        self.d_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            logits=self.real_logit - self.neg_logit, labels=tf.ones_like(self.real_logit)))
    ...
    def train_bpr(self, pos_content, pos_emb, neg_content, neg_emb, opp_emb):
        _, d_loss = self.sess.run([self.d_optimizer, self.d_loss],
                                  feed_dict={self.opp_emb: opp_emb,
                                             self.real_emb: pos_emb,
