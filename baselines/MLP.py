...
def build_mlp(mlp_in, hidden_dims, act, drop_rate, is_training, scope_name):
...
class MLP(object):
    def __init__(self, sess, args, emb_dim, content_dim):
        ...
        with tf.variable_scope("D"):
            # 是否需要先归一化
            self.user_emb = build_mlp(self.opp_emb, self.lys, self.act, self.dropout, self.is_training, 'user_emb')
            self.item_emb = build_mlp(self.real_emb, self.lys, self.act, self.dropout, self.is_training, 'item_emb')
