...
parser.add_argument('--bm', type=str, default="")
...
parser.add_argument('--samp_size', type=str, default='[25,25]', help='Sampling size of GNN')
...
parser.add_argument('--drop_emb_rate', type=float, default=0.5)
...
# load embedding
emb_path = os.path.join(args.datadir, args.dataset, "{}_{}-{}_u(sepdot)_i(sepdot)_plain.npy".format(
    args.embed_meth, args.samp_size[0], args.samp_size[1],
))
...
def get_exclude_pair(u_pair, ts_nei):
    if args.model != 'GAR':
        ...
        pos_item = np.array(list(set(para_dict['pos_user_nb'][u_pair[0]]) - set(ts_nei[u_pair[0]])),
                            dtype=np.int64)
...
model = eval("baselines_gnn.{}".format(args.model))(
    sess, args, emb.shape[-1], content_data.shape[-1])

# ===================== load trained dssm =====================
save_dir = './baselines_dssm/model_save/'

# load base model
if len(args.bm) > 1:
    bm_path = os.path.join(save_dir, f'{args.dataset}-GNN-{args.bm}')
    restore_var = [v for v in tf.all_variables() if 'D' in v.name]
    print("restored variables: ")
    pprint(restore_var)
    bm_saver = tf.train.Saver(restore_var)
    bm_saver.restore(sess, bm_path)
...
for epoch in range(1, args.max_epoch + 1):
    if args.loss == 'BPR':
        if args.model == 'GNN' and args.dataset == 'CiteULike':
            ...
            train_input = np.concatenate([train_data, neg_train_data], axis=1)
...
result_dir = './baselines_gnn/result/'
