...
model = eval("baselines_dssm.{}".format(args.model))(
...
# load base model
if len(args.bm) > 1:
    bm_path = os.path.join(save_dir, f'{args.dataset}-MLP-{args.bm}')
    # 想还原表现不能用 trainable_variables，要用 all_variables,
    # 因为还有 moving_mean 和 moving_variace 这些不可训练的参数
...
result_file = './baselines_dssm/result/'
