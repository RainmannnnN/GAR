...
class GAR(object):
    def __init__(self, sess, args, emb_dim, content_dim):
        ...
        real_input = tf.concat([self.real_emb, self.content], axis=-1)
        neg_input = tf.concat([self.neg_emb, self.content], axis=-1)
        gen_input = tf.concat([self.gen_emb, self.content], axis=-1)

        # D loss
        item_input = tf.concat([real_input, neg_input, gen_input], axis=0)
        D_out = self.build_discriminator(uemb, item_input, self.d_layer, self.d_act, self.d_drop, self.d_training,
                                         False)
        ...
        # G loss
        self.g_out = self.build_discriminator(self.opp_emb, gen_input,
                                              self.d_layer, self.d_act, self.d_drop,
                                              self.d_training, True)
        self.d_out = self.build_discriminator(self.opp_emb, real_input,
                                              self.d_layer, self.d_act, self.d_drop,
                                              self.d_training, True)
        # g_uemb = tf.tile(self.opp_emb, [2, 1])
        # g_iemb = tf.concat([real_input, gen_input], axis=0)
        # G_out = self.build_discriminator(g_uemb, g_iemb,
        #                                  self.d_layer, self.d_act, self.d_drop,
        #                                  self.d_training, True)
        # G_out = tf.transpose(tf.reshape(G_out, [2, -1]))
        # self.d_out = tf.gather(G_out, axis=1, indices=[0])
        # self.g_out = tf.gather(G_out, axis=1, indices=[1])
        ...
        with tf.control_dependencies(d_update_ops):
            self.d_optimizer = tf.train.AdamOptimizer(self.d_lr).minimize(
                self.d_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='D'))
        g_update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope='G')
        with tf.control_dependencies(g_update_ops):
            self.g_optimizer = tf.train.AdamOptimizer(self.g_lr).minimize(
                self.g_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='G'))
    ...
    def build_discriminator(self, uembs, iembs, hid_dims, act, drop_rate, training, reuse):
        with tf.variable_scope("D", reuse=reuse):
            out_uemb = build_mlp(uembs, hid_dims, act, drop_rate, training, 'user_emb', bn_first=False)
            out_iemb = build_mlp(iembs, hid_dims, act, drop_rate, training, 'item_emb', bn_first=False)
    ...
    def get_item_emb(self, content, item_emb, warm_item, cold_item):
        ...
        out_emb[cold_item] = self.sess.run(self.gen_emb, feed_dict={self.content: content[cold_item],
                                                                    self.g_training: False})
        out_emb = np.concatenate([out_emb, content], axis=-1)
